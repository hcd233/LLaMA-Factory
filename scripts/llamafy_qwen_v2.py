...
from transformers.modeling_utils import SAFE_WEIGHTS_INDEX_NAME, SAFE_WEIGHTS_NAME, WEIGHTS_INDEX_NAME, WEIGHTS_NAME, shard_checkpoint
...
def save_weight(input_dir: str, output_dir: str, shard_size: str, save_safetensors: bool) -> str:
    ...
    for key, value in tqdm(qwen_state_dict.items(), desc="Convert format"):
        ...
        if "self_attn.o_proj" in key:
            ...
            llama2_state_dict[key.replace(".weight", ".bias")] = torch.zeros_like(value[:, 0]).squeeze()
        else:
            llama2_state_dict[key] = value
...
def llamafy_qwen_v2(input_dir: str, output_dir: str, shard_size: Optional[str] = "2GB", save_safetensors: Optional[bool] = False):
    try:
